...
export class CourseCrudService {
  ...
  createLesson(lesson: LessonDetail): Observable<LessonDetail> {
    ...
    return this.apiService.createLesson(lessonPayload).pipe(
      tap(createdLesson => {
        // Create the full lesson object with the computed sort order
        const fullLesson = this.courseStateCoordination.createFullLessonEntity(
          lesson, 
          createdLesson, 
          computedSortOrder
        );
        
        console.log(`[CourseCrudService] Adding lesson to state with sortOrder: ${fullLesson.sortOrder}`);
        
        // Add to state immediately with correct data
