// **SIMPLIFIED FIX** - ScheduleApiService - State-First Pattern
// PATTERN: Check state for schedule ID → Get ID if missing → Use ID for all operations
// ENHANCEMENT: Single method that handles ID retrieval internally
...
export class ScheduleApiService {
  ...
  getScheduleEventsByDateRange(scheduleId: number, startDate: Date, endDate: Date): Observable<ScheduleEvent[]> {
    ...
    return this.http.get<ScheduleEvent[]>(`${this.apiUrl}/Schedule/${scheduleId}/events/dateRange`, {
      ...
      map(response => this.transformResponse<ScheduleEvent[]>(response)),
      catchError((error) => {
        console.error(`[ScheduleApiService] Failed to fetch schedule events for schedule ${scheduleId}:`, error.message);
  ...
  getDebugInfo(): any {
    return {
      ...
      pattern: 'State-first: Always use schedule ID from state',
